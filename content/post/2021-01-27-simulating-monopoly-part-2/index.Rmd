...
Generally speaking, we see that properties with higher rent tend to have higher expected income and almost looks like it follows the pattern of the board itself. However there are a few differences. Vine Street (£0.49) has the same expected income than Fleet Street (£0.49), despite being in the "inferior" set. Moreover, Trafalgar Square (£0.64) is superior to all of Yellow's expected income.
...
For Brown, it requires significantly more oppositions rolls before generating a profit. It only becomes competitive with other sets when it has a hotel built.
...
From the 2017 edition onwards, the maximum number of players in a game of monopoly is 8[\^1]. This means there are 7 opponents in the game. The minimun is 2, meaning there is 1 opponent.
...
The table shows how many players turns it will take for a set and property type to pay itself off depending on the number of opponents
